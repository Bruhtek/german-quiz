...
let questionAnswerAbcd = document.querySelector(
	".question-container .answer .abcd"
);
let questionAnswerWrite = document.querySelector(
	".question-container .answer .write"
);
let writeSubmitBtn = document.querySelector(
	".question-container .answer .write .write-submit-btn"
);
let writeInput = document.querySelector(
	".question-container .answer .write input"
);

abcdAnswers[0].addEventListener("click", (event) => {
	selectAnswer(0);
});
abcdAnswers[1].addEventListener("click", (event) => {
	selectAnswer(1);
});
abcdAnswers[2].addEventListener("click", (event) => {
	selectAnswer(2);
});
abcdAnswers[3].addEventListener("click", (event) => {
	selectAnswer(3);
});

writeSubmitBtn.addEventListener("click", (event) => {
	submitAnswer();
});
...
function submitAnswer() {
	...
	if (
		answer.toLowerCase() ==
		questions[currentQuestionIndex].correct.toLowerCase()
	) {
...
async function dontReplay() {
	let response = await fetch("/quiz/scores");
	let data = await response.json();
	console.log(data[nameInput.value]);
	if (data[nameInput.value]) {
		quizAnswers = data[nameInput.value].answers;
		correctCount = data[nameInput.value].correct;
		wrongCount = data[nameInput.value].wrong;

		updateScore();
		
		currentQuestionIndex = quizAnswers.length;
	}
}
